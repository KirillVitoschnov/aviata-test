<template>
  <div class="filter-wrapper">
    <div @click="checkedItems=[]" class="tooltip-wrapper">
      <div class="tooltip">
        ...
        <span class="tooltiptext">Сбросить выбор</span>
      </div>
    </div>
    <h1>{{ title }}</h1>
    <div class="filter-scroll">
      <div class="checkbox-wrapper" v-for="(key,index) in keys" :key="key">
        <input
            ...
            name="subscribe"
            :value="key"/>
        <label :for="key">{{ values[index] }}</label>
      </div>
    </div>
